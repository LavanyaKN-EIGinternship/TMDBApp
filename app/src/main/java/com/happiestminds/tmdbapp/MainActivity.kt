            ...
            if(response.isSuccessful){
                response.body()?.let {
                    ...
                    rView.adapter = MovieAdapter(it.results){
                        val intent = Intent(this@MainActivity, ActionActivity::class.java)
                        intent.putExtra("movie", it)
                        startActivity(intent)
                    }
                }
            }else{
                Toast.makeText(this@MainActivity, "There seems some problem at the backend, Please try later",
